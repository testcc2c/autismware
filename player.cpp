...
void Hooks::UpdateClientSideAnimation() {
	...
	//CBoneAccessor* accessor = &g_cl.m_local->m_BoneAccessor();
	vec3_t vecBackupAbsOrigin = g_cl.m_local->GetAbsOrigin();
	ang_t vecBackupAbsAngles = g_cl.m_local->GetAbsAngles();
	...
	g_cl.m_local->SetAbsOrigin(vecBackupAbsOrigin);
	g_cl.m_local->SetAbsAngles(vecBackupAbsAngles);
