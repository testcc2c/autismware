...
void Visuals::DrawProjectile(Weapon* ent) {
	...
	// draw smoke.
	if (ent->is(HASH("CSmokeGrenadeProjectile"))) {
	...
	Color colproj = g_menu.main.visuals.proj_range_color.get();
	Color colproj2 = g_menu.main.visuals.proj_range_color2.get();
	...
	// find classes.
		...
		if (g_menu.main.visuals.proj_range.get(0)) {
			render::Draw3DRainbowCircle(origin, 165.f, colproj);
		// circle timer bar
		...
		if (g_menu.main.visuals.proj_range.get(1)) {
			render::Draw3DFilledCircle(origin, 144, colproj2);
		// circle timer bar
			render::DrawFilledCircle(screen.x, screen.y - 7, 22, 180, Color(26, 26, 30, col.a()));
			render::DrawFilledCircle(screen.x, screen.y - 7, 22, 180, Color(26, 26, 30, col.a()));
...
void Visuals::Hitmarker() {
	...
	render::rect_filled(x / 2 + 6, y / 2 + 6, 1, 1, { 66, 245, 87, alpha });
	render::rect_filled(x / 2 + 7, y / 2 + 7, 1, 1, { 66, 245, 87, alpha });
	render::rect_filled(x / 2 + 8, y / 2 + 8, 1, 1, { 66, 245, 87, alpha });
	render::rect_filled(x / 2 + 9, y / 2 + 9, 1, 1, { 66, 245, 87, alpha });
	render::rect_filled(x / 2 + 10, y / 2 + 10, 1, 1, { 66, 245, 87, alpha });

	render::rect_filled(x / 2 - 6, y / 2 - 6, 1, 1, { 245, 66, 206, alpha });
	render::rect_filled(x / 2 - 7, y / 2 - 7, 1, 1, { 245, 66, 206, alpha });
	render::rect_filled(x / 2 - 8, y / 2 - 8, 1, 1, { 245, 66, 206, alpha });
	render::rect_filled(x / 2 - 9, y / 2 - 9, 1, 1, { 245, 66, 206, alpha });
	render::rect_filled(x / 2 - 10, y / 2 - 10, 1, 1, { 245, 66, 206, alpha });

	render::rect_filled(x / 2 - 6, y / 2 + 6, 1, 1, { 245, 102, 66, alpha });
	render::rect_filled(x / 2 - 7, y / 2 + 7, 1, 1, { 245, 102, 66, alpha });
	render::rect_filled(x / 2 - 8, y / 2 + 8, 1, 1, { 245, 102, 66, alpha });
	render::rect_filled(x / 2 - 9, y / 2 + 9, 1, 1, { 245, 102, 66, alpha });
	render::rect_filled(x / 2 - 10, y / 2 + 10, 1, 1, { 245, 102, 66, alpha });

	render::rect_filled(x / 2 + 6, y / 2 - 6, 1, 1, { 102, 181, 29, alpha });
	render::rect_filled(x / 2 + 7, y / 2 - 7, 1, 1, { 102, 181, 29, alpha });
	render::rect_filled(x / 2 + 8, y / 2 - 8, 1, 1, { 102, 181, 29, alpha });
	render::rect_filled(x / 2 + 9, y / 2 - 9, 1, 1, { 102, 181, 29, alpha });
	render::rect_filled(x / 2 + 10, y / 2 - 10, 1, 1, { 102, 181, 29, alpha });
	...
	for (int i{ 1 }; i <= g_csgo.m_globals->m_max_clients; ++i) {
		...
		if (render::WorldToScreen(g_shots.iPlayermins, g_shots.iPlayerbottom) && render::WorldToScreen(g_shots.iPlayermaxs, g_shots.iPlayertop))
		{
			...
			if (g_menu.main.misc.hittext.get()) {
				// text damage
				if (!g_shots.iHeadshot)
					render::indicator.string(box.x - alpha + box.w / 2, box.y - alpha - render::esp.m_size.m_height - 10, { 255, 0, 0, alpha }, out, render::ALIGN_CENTER);

				if (g_shots.iHeadshot == true)
					render::indicator.string(box.x - alpha + box.w / 2, box.y - alpha - render::esp.m_size.m_height - 20, { g_csgo.RandomInt(1,255) , g_csgo.RandomInt(1,255), g_csgo.RandomInt(1,255), alpha }, tfm::format(XOR("Headshot")), render::ALIGN_CENTER);
			}
		}
	}
}
...
void Visuals::StatisIndicators() {
	...
	// AUTO PEEK
		if (g_menu.main.visuals.indicators.get(5) && g_input.GetKeyState(g_menu.main.movement.autopeek.get())) {
			Indicator_t ind{ };
	...
	// OVERRIDE DMG
		int pistolsdmg = g_menu.main.aimbot.override_dmg_value_pistols.get();
		int scoutdmg = g_menu.main.aimbot.override_dmg_value_scout.get();
		int autodmg = g_menu.main.aimbot.override_dmg_value_auto.get();
		int awpdmg = g_menu.main.aimbot.override_dmg_value_awp.get();
		bool pistols = g_cl.m_weapon_id == GLOCK || g_cl.m_weapon_id == P2000 || g_cl.m_weapon_id == USPS || g_cl.m_weapon_id == ELITE || g_cl.m_weapon_id == P250 || g_cl.m_weapon_id == TEC9 || g_cl.m_weapon_id == CZ75A || g_cl.m_weapon_id == DEAGLE || g_cl.m_weapon_id == REVOLVER;
		bool autos = g_cl.m_weapon_id == G3SG1 || g_cl.m_weapon_id == SCAR20;
		if (g_menu.main.visuals.indicators.get(4) && g_aimbot.m_damage_toggle) {
		Indicator_t ind{ };
		ind.color = g_aimbot.m_damage_toggle ? 0xff15c27b : 0xffffffff;
		if (pistolsdmg > 100.f && pistols)
			ind.text = tfm::format(XOR("DMG - HP+%i"), pistolsdmg - 100.f);
		else if (pistols)
			ind.text = tfm::format(XOR("DMG - %i"), pistolsdmg);
		if (scoutdmg > 100.f && g_cl.m_weapon_id == SSG08)
			ind.text = tfm::format(XOR("DMG - HP+%i"), scoutdmg - 100.f);
		else if (g_cl.m_weapon_id == SSG08)
			ind.text = tfm::format(XOR("DMG - %i"), scoutdmg);
		if (autodmg > 100.f && autos)
			ind.text = tfm::format(XOR("DMG - HP+%i"), autodmg - 100.f);
		else if (autos)
			ind.text = tfm::format(XOR("DMG - %i"), autodmg);
		if (awpdmg > 100.f && g_cl.m_weapon_id == AWP)
			ind.text = tfm::format(XOR("DMG - HP+%i"), awpdmg - 100.f);
		else if (g_cl.m_weapon_id == AWP)
			ind.text = tfm::format(XOR("DMG - %i"), awpdmg);
		indicators.push_back(ind);
	}
	...
	if (last_lby[local_player->index()] != local_player->m_flLowerBodyYawTarget())
	{
		...
		next_lby_update[local_player->index()] = curtime + 1.1f + g_csgo.m_globals->m_interval;
	}

	if (next_lby_update[local_player->index()] < curtime)
	{
		next_lby_update[local_player->index()] = curtime + 1.1f;
	}
...
void Visuals::Spectators() {
	...
	/*static vec2_t					  main_mouse2, last_mouse2, s_drag2;
	bool                              is_dragging2;
	vec2_t							  position2 = vec2_t(100, 100);

	// get mouse position.
	g_csgo.m_surface->gain_mouse_pos(main_mouse2, last_mouse2);

	// dragging logic.
	if (main_mouse2.x > position2.x - s_drag2.x && main_mouse2.y > position2.y - s_drag2.y && main_mouse2.x < (position2.x - s_drag2.x) + 200 && main_mouse2.y < (position2.y - s_drag2.y) + 20 && GetAsyncKeyState(VK_LBUTTON)) {
		s_drag2.x += main_mouse2.x - last_mouse2.x;
		s_drag2.y += main_mouse2.y - last_mouse2.y;
		is_dragging2 = true;
	}

	// set our new position.
	position2.x -= s_drag2.x;
	position2.y -= s_drag2.y;

	//choked commands
	float choke_value = g_csgo.m_cl->m_choked_commands;

	render::indicator.string(position2.x + 98, position2.y + 61, { g_menu.main.misc.menu_color.get().r(), g_menu.main.misc.menu_color.get().g(), g_menu.main.misc.menu_color.get().b(),255 }, "Choke", render::ALIGN_CENTER);

	// outline
	render::rect_filled(position2.x + 50, position2.y + 75, 97, 6, { 10, 10, 10, 160 });
	// line.
	render::rect_filled(position2.x + 50, position2.y + 75, choke_value * (97 / 16.f), 6, { g_menu.main.misc.menu_color.get().r(),  g_menu.main.misc.menu_color.get().g(),  g_menu.main.misc.menu_color.get().b(), 255 });
	*/
}
...
void Visuals::DrawPlayer(Player* player) {
	...
	if (health_esp) {
		int y = box.y + 1;
		int h = box.h - 2;
		if (g_menu.main.players.healthbartype.get(1)) {
			Color clr = g_menu.main.players.name_color.get();
			int hp = std::min(100, player->m_iHealth());
			render::esp.string(box.x + box.w / 2, box.y - 10 - render::esp.m_size.m_height, clr, std::to_string(hp), render::ALIGN_CENTER);
		}
		if (g_menu.main.players.healthbartype.get(0)) {
	...
	// draw flags.
	{
		...
		if (g_menu.main.players.flags_enemy.get(6) && data && data->m_records.size() && g_cl.m_processing)
		{
			LagRecord* current = data->m_records.front().get();
			if (current)
				flags.push_back({ current->m_resolver_mode2, {0, 255, 255, low_alpha} });
		}
	...
	// draw bottom bars.
	{
		...
		// draw lby update bar.
		if (enemy && g_menu.main.players.lby_update.get() && g_cl.m_processing) {
			AimPlayer* data = &g_aimbot.m_players[player->index() - 1];
...
void Visuals::Hitmarker3D()
{
	...
	for (int i = 0; i < hitmarkers.size(); i++) {
		...
		render::line(pos2D.x + 2, pos2D.y + 2, pos2D.x + 5, pos2D.y + 5, { r, g, b, 255 });
		render::line(pos2D.x - 2, pos2D.y - 2, pos2D.x - 5, pos2D.y - 5, { r, g, b, 255 });
		render::line(pos2D.x + 2, pos2D.y - 2, pos2D.x + 5, pos2D.y - 5, { r, g, b, 255 });
		render::line(pos2D.x - 2, pos2D.y + 2, pos2D.x - 5, pos2D.y + 5, { r, g, b, 255 });
	}
