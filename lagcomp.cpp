...
void LagCompensation::Extrapolate(Player* player, vec3_t& origin, vec3_t& velocity, int& flags, bool wasonground)
{
	static ConVar* sv_gravity = g_csgo.m_cvar->FindVar(HASH("sv_gravity"));
	static ConVar* sv_jump_impulse = g_csgo.m_cvar->FindVar(HASH("sv_jump_impulse"));

	if (!(flags & FL_ONGROUND))
		velocity.z -= (g_csgo.m_globals->m_frametime * sv_gravity->GetFloat());
	else if (wasonground)
	...
	vec3_t end = src + (velocity * g_csgo.m_globals->m_frametime);
	...
	if (trace.m_fraction != 1.f)
	{
		for (int i = 0; i < 2; i++)
		{
			...
			g_csgo.m_engine_trace->TraceRay(Ray(trace.m_endpos, end, mins, max), MASK_SOLID, &filter, &trace);
	...
	g_csgo.m_engine_trace->TraceRay(Ray(origin, end, mins, max), MASK_SOLID, &filter, &trace);
...
void LagCompensation::PredictAnimations(CCSGOPlayerAnimState* state, LagRecord* record) {
	...
	LagRecord* pPreviousRecord = data->m_records[1].get();
	vec3_t vecPreviousOrigin = pPreviousRecord->m_origin;
	int fPreviousFlags = pPreviousRecord->m_flags;

	/* could be crashing. just added. */
	Extrapolate(player, vecPreviousOrigin, player->m_vecVelocity(), player->m_fFlags(), fPreviousFlags & FL_ONGROUND);
	...
	// rerun the pose correction cuz we are setting them up again.
