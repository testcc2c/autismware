...
void events::round_start(IGameEvent* evt) {
	...
	bool enable = g_menu.main.misc.buybotenable.get();
	// buybot.
	if (enable) {
		...
		g_csgo.m_engine->ExecuteClientCmd(tfm::format(XOR("buy vest")).data()); // always buy armor before nades and other shit for pistol round.
...
void events::player_hurt(IGameEvent* evt) {
	...
	if (evt && g_menu.main.misc.notifications.get(1)) {
		...
		if (attacker != g_cl.m_local && victim == g_cl.m_local) {
			...
			g_notify.add(tfm::format(XOR("Got hit by %s in the %s for %i damage\n"), name, g_shots.m_groups[evt->m_keys->FindKey(HASH("hitgroup"))->GetInt()], damage));
		}
