...
namespace math {
    ...
    __forceinline float YawDelta(float destAngle, float srcAngle) {
        float delta;
    ...
    vec3_t VectorAngles(const vec3_t& v);

    vec3_t AngleVectors(const vec3_t& angles);
    ...
    void  AngleVectors(const ang_t& angles, vec3_t& forward, vec3_t& right, vec3_t& up);
