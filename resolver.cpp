...
void Resolver::SetMode(LagRecord* record)
{
	...
	if ((record->m_flags & FL_ONGROUND) && (speed < 0.1f || record->m_fake_walk || record->m_fake_flick) && !g_input.GetKeyState(g_menu.main.aimbot.resolver_override.get()))
	{
		record->m_eye_angles.x = 89.f;
		record->m_mode = Modes::RESOLVE_STAND;
	}
	// override
	if (g_input.GetKeyState(g_menu.main.aimbot.resolver_override.get()) && record->m_flags & FL_ONGROUND && (speed < 0.1f || record->m_fake_walk))
...
void Resolver::ResolveAngles(Player* player, LagRecord* record) 
{
	...
	else if (record->m_mode == Modes::RESOLVE_STAND && fake && !(g_input.GetKeyState(g_menu.main.aimbot.resolver_override.get())))
...
void Resolver::Freestand(Player* player, AimPlayer* data, LagRecord* record)
{
	...
	for (auto it = angles.begin(); it != angles.end(); ++it) {
		...
		for (float i{ 0.f }; i < len; i += STEP) {
			...
			if (i < (len * 0.05f)) { // was originally .45 when i pasted this, but .05 works so much better.
				mult = 1.25f;
				record->m_eye_angles.y = away - 180.f; // put head back if most of their body is out.
				return;
	...
	if (!record->m_fake_flick && best->m_yaw - record->m_body > 90.f || best->m_yaw - record->m_body < -90.f)
...
void Resolver::ResolveStand(Player* player, AimPlayer* data, LagRecord* record)
{
	...
	{
		if (ShouldUseFreestand(record) && data->m_freestand_index < 2) // if not in open.
		{
		...
		else if (data->m_moved && data->m_brute_index < 2) // brute based off last move.
		{
			...
			case 0:
				record->m_eye_angles.y = move->m_body - 35.f;
				break;
...
void Resolver::ResolveAir(Player* player, AimPlayer* data, LagRecord* record)
{
	...
	case 0:
		record->m_eye_angles.y = math::normalize_float(away - 180.f);
		break;
