...
void HVH::AntiAimPitch() {
	...
	if (state->m_speed > 0.1f || !state->m_ground) {
		canup = false;
	}
	else {
		canup = true;
	...
	bool upolu = g_menu.main.antiaim.uppitchonlby.get();
	...
	case 3:
		// fake up
		g_cl.m_cmd->m_view_angles.x = -991.0f;
		break;
	case 4:
		// fake down
		g_cl.m_cmd->m_view_angles.x = 991.0f;
...
void HVH::GetAntiAimDirection() {
	bool manualaa = g_hvh.m_left || g_hvh.m_right || g_hvh.m_back;
	// edge aa.
	if (g_menu.main.antiaim.edge.get() == 1 && g_cl.m_local->m_vecVelocity().length() < 320.f && !manualaa) {
	...
	// reverse edge aa.
	if (g_menu.main.antiaim.edge.get() == 2 && g_cl.m_local->m_vecVelocity().length() < 320.f && !manualaa && g_cl.m_speed == 0.f) {
		ang_t ang;

		if (g_csgo.m_globals->m_curtime + 0.3 >= g_cl.m_body_pred)
		{
			if (DoEdgeAntiAim(g_cl.m_local, ang))
			{
				m_direction = ang.y;
...
void HVH::DoRealAntiAim() {
	...
	// fix cancer spectate
	if (!g_cl.m_processing && !g_cl.m_cmd->m_view_angles.z == 0.f)
	...
	if (m_yaw > 0) {
		...
		bool disable = (g_input.GetKeyState(g_menu.main.movement.fakewalk.get()) && g_menu.main.antiaim.disablelbyfw.get());
		// check if we will have a lby fake this tick.
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air) && !disable) {
			...
			if (stand) {
				switch (g_menu.main.antiaim.body_fake_stand.get()) {
					...
					// break logic.
				case 1:
					if (g_menu.main.antiaim.edge.get() == 1 && g_menu.main.antiaim.targetbreak.get() && DoEdgeAntiAim(g_cl.m_local, ang)) {
						if (best_target) {
							ang_t angle;
							math::VectorAngles(best_target->m_vecOrigin() - g_cl.m_local->m_vecOrigin(), angle);
				...
				case 2:
					if (g_menu.main.antiaim.edge.get() == 1 && g_menu.main.antiaim.targetbreak.get() && DoEdgeAntiAim(g_cl.m_local, ang)) {
						...
						if (g_menu.main.antiaim.sideselect.get() == 0)
							g_cl.m_cmd->m_view_angles.y -= 64.f;
						else
							g_cl.m_cmd->m_view_angles.y += 64.f;
				...
				case 3:
					if (g_menu.main.antiaim.edge.get() == 1 && g_menu.main.antiaim.targetbreak.get() && DoEdgeAntiAim(g_cl.m_local, ang)) {
						...
						negative ? g_cl.m_cmd->m_view_angles.y += g_csgo.RandomInt(-110.f, 0.f) : g_cl.m_cmd->m_view_angles.y += g_csgo.RandomInt(0.f, 90.f);
						negative = !negative;
					}
					break;
				...
				case 4:
					if (g_menu.main.antiaim.edge.get() == 1 && g_menu.main.antiaim.targetbreak.get() && DoEdgeAntiAim(g_cl.m_local, ang)) {
						...
						negative ? g_cl.m_cmd->m_view_angles.y -= 119.f : g_cl.m_cmd->m_view_angles.y -= 25.f;
						negative = !negative;
					}
					break;
				...
				case 5: {
					if (g_menu.main.antiaim.edge.get() == 1 && g_menu.main.antiaim.targetbreak.get() && DoEdgeAntiAim(g_cl.m_local, ang)) {
						...
						g_cl.m_cmd->m_view_angles.y = m_direction + 90.f + std::fmod(g_csgo.m_globals->m_curtime * -60.f, -360.f);
					}
					break; 
	...
	if (!g_input.GetKeyState(g_menu.main.movement.cripwalk.get())) {
		...
		case 3: {
			...
			break;
			  ...
			  // rotate.
		case 4: {
			g_cl.m_cmd->m_view_angles.y = m_direction + 90.f + std::fmod(g_csgo.m_globals->m_curtime * 270.f, -180.f);
			break;
		}
		...
		case 7: {
			static int negative = false;
			g_cl.m_cmd->m_view_angles.y = m_direction + 180.f;
			negative ? g_cl.m_cmd->m_view_angles.y += 110 + std::fmod(g_csgo.m_globals->m_curtime * -75.f, 90.f) : g_cl.m_cmd->m_view_angles.y -= 90 + std::fmod(g_csgo.m_globals->m_curtime * -110.f, 130.f);
			negative = !negative;
			break;
		}
			  // break.
		case 8: {
			...
			g_cl.m_cmd->m_view_angles.y = m_direction + 180.f;
			if (switcher)
				g_cl.m_cmd->m_view_angles.y += g_csgo.RandomFloat(-58.f, 58.f);
			else
				g_cl.m_cmd->m_view_angles.y -= g_csgo.RandomFloat(-58.f, 58.f);
			switcher = !switcher;
			break;
		...
		case 9: {
			static int negative = false;
			negative = !negative;
			g_cl.m_cmd->m_view_angles.y = m_direction + 0.f;
			negative ? g_cl.m_cmd->m_view_angles.y += 30 + std::fmod(g_csgo.m_globals->m_curtime * 120.f, -58.f) : g_cl.m_cmd->m_view_angles.y -= 20 + std::fmod(g_csgo.m_globals->m_curtime * 120.f, -45.f);
			break;
		}
...
void HVH::Distort() {
	...
	if (!g_menu.main.antiaim.distortion.get() || g_input.GetKeyState(g_menu.main.memes.fakeflick.get()))
		return;
	...
	auto distortspeed = g_menu.main.antiaim.distortspeed.get();
...
bool IsPressingMovementKeys(CUserCmd* cmd)
{
	...
	if (cmd->m_buttons & IN_FORWARD || cmd->m_buttons & IN_BACK || cmd->m_buttons & IN_LEFT || 
		cmd->m_buttons & IN_RIGHT || cmd->m_buttons & IN_MOVELEFT || cmd->m_buttons & IN_MOVERIGHT)
		return true;
...
void HVH::AntiAim() {
	...
	if (g_input.GetKeyState(g_menu.main.memes.fakeflick.get())) //dopium fakeflick
	{
		bool manualaa = g_hvh.m_left || g_hvh.m_right || g_hvh.m_back;
		if (g_cl.m_local->m_MoveType() != MOVETYPE_LADDER && !IsPressingMovementKeys(g_cl.m_cmd) && !manualaa) {
			size_t speed = 20;

			if (g_cl.m_cmd->m_tick % speed == speed / 2) {
				*g_cl.m_packet = false;
				g_cl.m_cmd->m_view_angles.y += 110.f;
			}
			else if (g_cl.m_cmd->m_tick % speed == speed)
			{
				*g_cl.m_packet = false;
				g_cl.m_cmd->m_view_angles.y += 35.f;
				g_cl.m_cmd->m_forward_move = g_cl.m_cmd->m_tick % 3 ? 400.f : -400.f;
			}
			else if (g_cl.m_cmd->m_tick % speed == 0)
			{
				*g_cl.m_packet = false;
		...
		else if (g_cl.m_local->m_MoveType() != MOVETYPE_LADDER && !IsPressingMovementKeys(g_cl.m_cmd) && !g_hvh.m_left) {
		...
		else if (g_cl.m_local->m_MoveType() != MOVETYPE_LADDER && !IsPressingMovementKeys(g_cl.m_cmd) && !g_hvh.m_right) {
...
void HVH::SendPacket() {
	...
	else
	{
		...
		if (g_menu.main.antiaim.lag_enable.get() && !g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN)) {
			...
			for (auto it = activation.begin(); it != activation.end(); it++) {
				...
				// move.
				if (*it == 1 && delta > 0.1f && g_cl.m_speed > 0.1f && (g_cl.m_flags & FL_ONGROUND)) {
					active = true;
					break;
				}
			...
			if (active) {
				...
				{
					if (g_cl.m_cmd->m_tick % 40 < 20)
					{
						limit = g_menu.main.antiaim.lag_limit.get();
					}
					else
						limit = 2;
	...
	static bool switcher = false;
	float val1 = g_csgo.RandomFloat(-180.f, 180.f);
	float val2 = g_csgo.RandomFloat(-180.f, 180.f);
	if (g_menu.main.memes.cancerspec.get()) {
		if (g_cl.m_speed < 0.1f && !g_input.GetKeyState(g_menu.main.movement.fakewalk.get()))
		{
			if (switcher)
				g_cl.m_cmd->m_view_angles.z = val1;
			else
				g_cl.m_cmd->m_view_angles.z = val2;
			switcher = !switcher;
		}
		else
			g_cl.m_cmd->m_view_angles.z = 0.f;
