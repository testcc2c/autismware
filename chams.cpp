...
void Chams::init() {
	...
	// ty nugsy.
	std::ofstream("csgo/materials/animated_wireframe.vmt") << R"#(" VertexLitGeneric "
	{
		        "$basetexture"				"dev/zone_warning"
		        "$additive"					"1"
		        "$envmap"					"editor/cube_vertigo"
		        "$envmaptint"				"[0 0.5 0.75]"
		        "$envmapfresnel"			"1"
	...
	std::ofstream("csgo/materials/custom_overlay.vmt") << R"#("VertexLitGeneric"
		    {
	...
	materialMetallnZ = g_csgo.m_material_system->FindMaterial("promethea_metallic", XOR("Model textures"));
	...
	skeet = g_csgo.m_material_system->FindMaterial(XOR("glowOverlay"), XOR("Model textures"));
	skeet->IncrementReferenceCount(); // glow

	animated_wireframe = g_csgo.m_material_system->FindMaterial(XOR("custom_overlay"), nullptr);
	animated_wireframe->IncrementReferenceCount();

	onetap = g_csgo.m_material_system->FindMaterial(XOR("onetap_overlay"), XOR("Model textures"));
	onetap->IncrementReferenceCount(); // overlay or ghost
}
...
void Chams::RenderHistoryChams(int index) {
	...
	if (enemy && g_menu.main.players.chams_enemy_history_enable.get()) {
		...
		case 6:
			...
			break;
		case 7:
			SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_enemy_history_wireframe.get());
			if (g_menu.main.players.chams_local_mat.get() == 7)
			{
				SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_enemy_history_wireframe.get());
				player->DrawModel();
			}
...
void Chams::RenderFake() {
	if (g_menu.main.antiaim.draw_angles_chams.get()) { // check if active
		if (g_cl.m_local) {
			Color color = g_menu.main.antiaim.color_draw_angles_chams.get();
			...
			SetupMaterial(skeet, g_menu.main.antiaim.color_draw_angles_chams.get(), false, g_menu.main.players.chams_enemy_history_wireframe.get());
			ang_t fake1337(0, g_cl.m_radar.y, 0);
			g_cl.m_local->SetAbsAngles(fake1337);
			g_cl.m_local->DrawModel();
			// fix color picker for skeet chams.
			g_chams.skeet->FindVar("$envmaptint", nullptr)->SetVecValue(color.r() / 255.f, color.g() / 255.f, color.b() / 255.f);
		}
	}
}
...
void Chams::RenderPlayer(Player* player) {
	...
	if (g_menu.main.players.chams_local_mat.get() == 7)
		g_menu.main.players.chams_local_wireframe.set(true);
	else
		g_menu.main.players.chams_local_wireframe.set(false);

	if (g_menu.main.players.chams_friendly_mat.get() == 7)
		g_menu.main.players.chams_friendly_wireframe.set(true);
	else
		g_menu.main.players.chams_friendly_wireframe.set(false);

	if (g_menu.main.players.chams_enemy_mat.get() == 7)
		g_menu.main.players.chams_enemy_wireframe.set(true);
	else
		g_menu.main.players.chams_enemy_wireframe.set(false);
	...
	if (player->m_bIsLocalPlayer()) {
		if (g_menu.main.players.chams_local_scope.get() && player->m_bIsScoped())
			...
			case 4:
				...
				break;
			case 5:
				SetupMaterial(skeet, color, false, g_menu.main.players.chams_local_wireframe.get());
				break;
			case 6:
				SetupMaterial(onetap, color, false, g_menu.main.players.chams_local_wireframe.get());
				break;
			case 7:
				SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_local_wireframe.get());
				if (g_menu.main.players.chams_local_mat.get() == 7)
				{
					SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_local_wireframe.get());
					player->DrawModel();
		...
		// manually draw the model.
		if (!g_menu.main.memes.houdini.get())
			player->DrawModel();
	}
	...
	if (enemy && g_menu.main.players.chams_enemy.get(0) && g_menu.main.players.chams_enemy_enable.get()) {
		if (g_menu.main.players.chams_enemy.get(1)) {
			...
			case 4:
				...
				break;
			case 5:
				SetupMaterial(skeet, color, true, g_menu.main.players.chams_enemy_wireframe.get());
				break;
			case 6:
				SetupMaterial(onetap, color, true, g_menu.main.players.chams_enemy_wireframe.get());
				break;
			case 7:
				SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_enemy_wireframe.get());
				if (g_menu.main.players.chams_local_mat.get() == 7)
				{
					SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_enemy_wireframe.get());
					player->DrawModel();
		...
		case 4:
			...
			break;
		case 5:
			SetupMaterial(skeet, color, false, g_menu.main.players.chams_enemy_wireframe.get());
			break;
		case 6:
			SetupMaterial(onetap, color, false, g_menu.main.players.chams_enemy_wireframe.get());
			break;
		case 7:
			SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_enemy_wireframe.get());
			if (g_menu.main.players.chams_local_mat.get() == 7)
	...
	/* friendly invsible chams */
		...
		if (g_menu.main.players.chams_friendly.get(1)) {
			...
			case 4:
				...
				break;
			case 5:
				SetupMaterial(skeet, color, true, g_menu.main.players.chams_friendly_wireframe.get());
				break;
			case 6:
				SetupMaterial(onetap, color, true, g_menu.main.players.chams_friendly_wireframe.get());
				break;
			case 7:
				SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_friendly_wireframe.get());
				if (g_menu.main.players.chams_local_mat.get() == 7)
				{
					SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_friendly_wireframe.get());
					player->DrawModel();
			...
			if (g_menu.main.memes.modelscale.get())
				player->m_flModelScale() = scale;
			else
				player->m_flModelScale() = 1.f;
		}
		...
		case 4:
			...
			break;
		case 5:
			SetupMaterial(skeet, color, false, g_menu.main.players.chams_friendly_wireframe.get());
			break;
		case 6:
			SetupMaterial(onetap, color, false, g_menu.main.players.chams_friendly_wireframe.get());
			break;
		case 7:
			SetupMaterial(animated_wireframe, color, false, g_menu.main.players.chams_friendly_wireframe.get());
			if (g_menu.main.players.chams_local_mat.get() == 7)
