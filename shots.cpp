...
void Shots::OnShotFire(Player* target, float damage, int bullets, LagRecord* record, int hitbox) {
	...
	for (int i{ }; i < bullets; ++i) {
		...
		if (target && i == 0) {
			/* NOTE: YOU HAVE TO DEFINE THESE HERE OR ELSE YOU FUCKING CRASH ON MANUAL LMAO */
			// setup log variables.
			int   choked_ticks = game::TIME_TO_TICKS(shot.m_target->m_flSimulationTime() - shot.m_record->m_sim_time) - 1;
			bool  choking_ticks = (choked_ticks >= 1 && !(choked_ticks == 0)) != g_csgo.m_globals->m_interval;
			// shot.m_record->m_layers ????
			...
			auto layerCount = shot.m_target->GetNumAnimOverlays();
			for (int i = 0; i < layerCount; ++i)
			{
				C_AnimationLayer* currentLayer = shot.m_target->GetAnimOverlay(i);
				cycle = shot.m_record->m_layers[6].m_cycle;
				previous_cycle = shot.m_record->m_layers[11].m_prev_cycle;
				weight = shot.m_record->m_layers[i].m_weight;
				weight_delta_rate = shot.m_record->m_layers[i].m_weight_delta_rate;
				sequence = shot.m_record->m_layers[i].m_sequence;
				playback_rate = shot.m_record->m_layers[i].m_playback_rate;
			}
...
void Shots::OnImpact(IGameEvent* evt) {
	...
	if (!trace.m_entity || !trace.m_entity->IsPlayer()) {
		out = tfm::format(XOR("Missed shot due to bad resolve with %s resolver\n"), current->m_resolver_mode);
	...
	// this is a miss due to wrong angles.
		...
		else if (mode == Resolver::Modes::RESOLVE_SPIN)
			++data->m_spin_index;
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	if (g_menu.main.misc.hitmarker.get()) {
		...
		case 9: PlaySoundA(reinterpret_cast<char*>(tap), nullptr, SND_ASYNC | SND_MEMORY); break;
		}
	}
	...
	if (g_menu.main.misc.notifications.get(0)) {
		AimPlayer* data = &g_aimbot.m_players[target->index() - 1];
		LagRecord* current = data->m_records.front().get();
		std::string out = tfm::format(XOR("Hit %s in the %s for %i damage with %s resolver (%i health remaining)\n"), name, m_groups[group], (int)damage, current->m_resolver_mode, hp);
		g_notify.add(out);
		...
		if (hp == 0 && g_menu.main.misc.killsay.get()) {
			...
			case 0:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Dont worry! I'm autistic!");
				else
			...
			case 1:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Tapped");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Haha I put the 'bot' in 'aimbot'!");

			case 2:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Damn dude you might need to spit shine that");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Go get autism at your local preschool!");

			case 3:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Autism is the way to go!");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Be autistic! not normal");

			case 4:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Stop being normal!");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Autism is the way to go!");

			case 5:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Autismware, the fastest way to go negative!");
				else
					g_csgo.m_engine->ExecuteClientCmd(XOR("say Stop being positive go negative with autismware!"));

			case 6:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd(XOR("say Do you have autism yet?"));
				else
					g_csgo.m_engine->ExecuteClientCmd("say Youve been infected with autism!");
			}
		}
	}
	...
	else if (mode == Resolver::Modes::RESOLVE_FREESTAND && data->m_freestand_index > 0)
		--data->m_freestand_index;

	else if (mode == Resolver::Modes::RESOLVE_BRUTEFORCE && data->m_brute_index > 0)
		--data->m_brute_index;

	else if (mode == Resolver::Modes::RESOLVE_AIR && data->m_air_index > 0)
		--data->m_air_index;

	else if (mode == Resolver::Modes::RESOLVE_LAST_LBY && data->m_moving_index > 0)
		--data->m_moving_index;

	else if (mode == Resolver::Modes::RESOLVE_SPIN && data->m_spin_index > 0)
		--data->m_spin_index;
