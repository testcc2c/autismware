...
void math::AngleVectors(const ang_t& angles, vec3_t& forward, vec3_t& right, vec3_t& up)
{
    ...
    SinCos(deg_to_rad(angles[1]), &sy, &cy);
    SinCos(deg_to_rad(angles[0]), &sp, &cp);
    SinCos(deg_to_rad(angles[2]), &sr, &cr);
...
vec3_t math::VectorAngles(const vec3_t& v) {
...
vec3_t math::AngleVectors(const vec3_t& angles) {
    float sp, sy, cp, cy;
